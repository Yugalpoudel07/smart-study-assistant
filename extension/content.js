...
function createPanel() {
  ...
  style.textContent = `
    #ssa-panel {
      position: fixed; top: 60px; right: 24px; width: 380px; max-height: 560px;
    ...
    #ssa-header { display: flex; align-items: center; justify-content: space-between; padding: 10px 14px; background: linear-gradient(180deg, rgba(34,211,238,0.06) 0%, transparent 100%); border-bottom: 1px solid rgba(255,255,255,0.06); cursor: grab; user-select: none; flex-shrink: 0; }
    ...
    #ssa-header-right button { width: 24px; height: 24px; border: none; background: rgba(255,255,255,0.05); color: #8b9bc0; border-radius: 6px; cursor: pointer; font-size: 12px; display: flex; align-items: center; justify-content: center; transition: all 0.15s; }
    ...
    .ssa-tab { padding: 6px 8px; font-size: 10.5px; font-weight: 500; color: #5a6a8a; background: transparent; border: 1px solid transparent; border-radius: 7px; cursor: pointer; transition: all 0.15s; font-family: inherit; }
    ...
    #ssa-actions { padding: 8px 14px 12px; border-top: 1px solid rgba(255,255,255,0.06); flex-shrink: 0; display: flex; gap: 8px; }
    ...
    .ssa-loading::after { content: ''; display: inline-block; width: 14px; height: 14px; border: 2px solid rgba(34,211,238,0.3); border-top-color: #22d3ee; border-radius: 50%; animation: ssa-spin 0.7s linear infinite; margin-left: 8px; vertical-align: middle; }
  ...
  document.addEventListener("mousemove", (e) => {
    ...
    let x = Math.max(0, Math.min(window.innerWidth - 100, e.clientX - offsetX));
    let y = Math.max(0, Math.min(window.innerHeight - 50, e.clientY - offsetY));
  ...
  panel.querySelector("#ssa-export-pdf").addEventListener("click", async () => {
    ...
    try {
      const resp = await fetch("http://127.0.0.1:8000/export-pdf", {
        ...
        body: JSON.stringify({ simplified: analysis.simplified || "", questions: analysis.questions || [], keywords: analysis.keywords || [], difficulty: analysis.difficulty || "" })
      });
    ...
    } catch { alert("Could not export PDF. Is the backend running?"); }
  });
...
async function deleteHistoryItem(index) {
  try {
    ...
    loadHistory();
  } catch { alert("Could not delete. Is the backend running?"); }
}
...
async function loadHistory() {
  ...
  try {
    ...
    historyCache = items;
    if (!items.length) { container.innerHTML = '<div class="ssa-empty">No history yet</div>'; return; }
    ...
    container.querySelectorAll(".ssa-history-item-content").forEach((el) => {
      el.addEventListener("click", () => openHistoryDetail(historyCache[parseInt(el.parentElement.dataset.index)]));
    });
    container.querySelectorAll(".ssa-history-delete").forEach((btn) => {
      btn.addEventListener("click", (e) => { e.stopPropagation(); deleteHistoryItem(parseInt(btn.dataset.deleteIndex)); });
    });
  } catch { container.innerHTML = '<div class="ssa-empty">Backend offline</div>'; }
}
...
function showLoading() {
  ...
  ["simplified","questions","keywords","difficulty"].forEach(tab => {
    p.querySelector(`#ssa-${tab}`).innerHTML = `<div class="ssa-loading">${tab.charAt(0).toUpperCase()+tab.slice(1)}</div>`;
  });
}

function showResults(result) {
  const p = createPanel();
  if (result.simplified) p.querySelector("#ssa-simplified").innerHTML = `<div class="ssa-text">${result.simplified}</div>`;
  if (result.questions?.length) p.querySelector("#ssa-questions").innerHTML = result.questions.map((q, i) => `<div class="ssa-q-item"><div class="ssa-q-num">${i+1}</div><div class="ssa-q-text">${q}</div></div>`).join("");
  if (result.keywords?.length) p.querySelector("#ssa-keywords").innerHTML = `<div class="ssa-kw-grid">${result.keywords.map(k => `<span class="ssa-kw-tag">${k}</span>`).join("")}</div>`;
